...
max_pooled_scores2 = torch.load('maxpooledScores.t7')

max_pooled_scores = torch.Tensor(4592, 20)
for i=1,4592 do
	for j=1,20 do
		max_pooled_scores[i][j] = max_pooled_scores2[i][j]
	end
end

true_scores2 = torch.load('classmap_test_voc07.t7')
true_scores3 = true_scores2.outmat
--[[
true_scores4 = torch.Tensor(4952, 20)
for i=1,4952 do
	for j=1,20 do
		true_scores4[i][j] = true_scores3[j][i]
	end
end
]]

true_scores = torch.Tensor(4592, 20)
for i=1,4592 do
	for j=1,20 do
		true_scores[i][j] = true_scores3[i][j]
	end
end


max_pooled_scores_modified_1 = torch.Tensor(4592, 20)
for i=1,4592 do
	for j=1,20 do
...
err = torch.zeros(4592,1) -- error for each image
...
err_min = torch.zeros(4592,1) -- error for each image
...
max_pooled_scores_modified_3 = torch.Tensor(4592, 20)
for i=1,4592 do
	for j=1,20 do
		if max_pooled_scores[i][j] >= 0 then
			max_pooled_scores_modified_3[i][j] = 1
		else
			max_pooled_scores_modified_3[i][j] = -1
		end
	end
...
max_pooled_scores_modified_2 = torch.zeros(4592, 20)
--alpha = torch.Tensor(1,1)
for i = 1,4592 do
	tempvec = max_pooled_scores[{i, {}}]
--	max, indx = torch.max(tempvec, (1))
--	alpha = torch.Tensor.double(indx)
	max = torch.max(tempvec)
	for j = 1,20 do
		if tempvec[j] == max then
			max_pooled_scores_modified_2[i][j] = 1
		end
	end
--	max_pooled_scores_modified_2[i][alpha] = 1
end
